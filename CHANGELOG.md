...
- **07-guide-reference** (wp-version 1.4.0): fixed a factual error stating sub-agents were limited to depth=1, corrected to 5 levels deep since v2.1.172. Added Claude Sonnet 5 as the new default model (v2.1.197, native 1M context) to the models table, a Dynamic Workflows (`ultracode`) section, references to 7 previously unlinked guide pages (agent-harness, memory-systems, hooks-events-reference, tools-reference, team-knowledge-base, practitioner-insights, agentic-tools), corrected the series count from 12 to 13 whitepapers (WP12 was missing), and synced the frontmatter/footer version to 3.41.1.
