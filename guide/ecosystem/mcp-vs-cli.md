...
| **Klavis AI / Strata** (Klavis-AI/klavis, YC X25) | Hosted catalog of 50+ MCP servers with enterprise OAuth; Strata attacks catalog-size cost directly via "progressive discovery" instead of exposing every tool upfront. | ~5.5K stars. Vendor-reported gains (+13% accuracy, 83%+ success on complex workflows) are not independently verified, treat as directional, not measured. Paid tiers from $79/mo. |
| **Arcade.dev** (ArcadeAI/arcade-mcp) | Open-source Python framework fronting 7,500+ ready-made tools across 81 MCP servers (Slack, Google Workspace, Salesforce). | Framework is open source; the hosted runtime is paid. Addresses tool count at the integration layer, not schema-token cost per session directly. |
